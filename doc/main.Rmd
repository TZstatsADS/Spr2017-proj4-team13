...
In this project, our team were assigned two papers that proposed two algorithms concerning name disambiguations and we implmented these two algorithms in R code and we have also proposed evaluation methods to compare these algorithms. During the development of ths project, we have observed several interesting trends. In this file, we will present our data reading, preporcessing as well as results from our evaluation process.
...
For each record in the dataset, there are some information we want to extract and store them in a regular form: canonical author id, coauthors, paper title, publication venue title. In our main.rmd file, you will find our programs for input of each data file which have been proprocessed by our functions stored in "dataclean.R" under the lib folder.
```{r, include=FALSE}
...
$$
For Paper 3
Construct our feature design for paper three with resepct to Coauthor, Title and Journal
```{r}
...
\end{aligned}
$$
We first evalue our algorithm using the gold standard，given the length of the code, we hide the chunk so please refer to our main.rmd for details
...
Then we use our own ways to evalute, this methods assignm a specific Author ID to each generated clusters by locating the Author ID with the maximum number of appearence in each cluster. Again, due to the lengthy code we have, we choose to hide the code and please refer to our main.rmd for detials.
...
In addition to the above evaluation, we have also observed several interesting trend for algoeithm introduced in paper 6 using c2 constraint.
```{r}
